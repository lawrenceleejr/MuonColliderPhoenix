...
class ViewerBase:
    ...
    def exportGLTFScene(self, gltfFileName="test.gltf", singleInstance=False, color=[0.1,0.1,0.1,1.0]):
        ...
        for k in self.instancePlacements:
            ...
            for p in self.instancePlacements[k]:
                ...
                # print(t[0])
                skip=False
                if float(t[0])<=0.1 and float(t[1])>=-0.100 and abs(float(t[2]))<2:
                    skip=True
                if math.sqrt(float(t[0])*float(t[0])+float(t[1])*float(t[1]))<0.1:
                    skip=False
                if skip:
                    continue
